...
async def get_incident_detail(run_id: str, incident_id: str):
    ...
    try:
        ...
        if incident['run_id'] != run_id:
            raise HTTPException(status_code=404, detail=f"Incident not found in run: {run_id}")
        ...
        explanation = IncidentExplanation(
            incident_title=explanation_data.get('incident_title', incident['title']),
            ...
            referenced_line_numbers=explanation_data.get('referenced_line_numbers', [])
        )
        ...
        logger.error(f"Failed to get incident {incident_id}: {e}", exc_info=True)
