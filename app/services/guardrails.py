...
def get_validated_explanation(
    ...
    fixed_response = fix_json_with_llm(json.dumps(raw_response), "\n".join(errors))
    
    if fixed_response:
        explanation, retry_errors = _validate_explanation(fixed_response, evidence)
        if explanation:
            logger.info("LLM explanation fixed and validated on retry")
            return explanation, True, validation_errors  # Include original errors for logging
        validation_errors.extend(retry_errors)
    
    # All attempts failed - use fallback
    logger.warning("LLM validation failed after retry, using fallback")
    explanation = generate_fallback_explanation(evidence, signature)
    validation_errors.append("Fell back to deterministic explanation after validation failures")
...
def _validate_explanation(
    ...
    if not isinstance(likely_causes, list):
        ...
        for i, cause in enumerate(likely_causes):
            ...
            if not isinstance(evidence_lines, list):
                errors.append(f"likely_causes[{i}].evidence_line_numbers must be a list")
            else:
                for line_num in evidence_lines:
                    if not isinstance(line_num, int):
                        errors.append(f"likely_causes[{i}] has non-integer line number: {line_num}")
                    elif line_num not in valid_line_numbers:
                        errors.append(f"likely_causes[{i}] cites invalid line number {line_num} (valid: {sorted(valid_line_numbers)})")
                    else:
    ...
    if not isinstance(raw.get("referenced_line_numbers"), list):
        ...
        if missing_refs:
            errors.append(f"referenced_line_numbers missing cited lines: {sorted(missing_refs)}")
        
        # Check referenced lines are valid
        invalid_refs = referenced - valid_line_numbers
        if invalid_refs:
            errors.append(f"referenced_line_numbers contains invalid lines: {sorted(invalid_refs)}")
    ...
    try:
        ...
        for error in e.errors():
            errors.append(f"Pydantic validation: {error['loc']} - {error['msg']}")
        return None, errors
